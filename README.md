Here is a complete, ready-to-use main.py file tailored for your Anritsu MS46122B ShockLine VNA.It includes:Barcode scanning (Serial No., Part No., Order No.)
VNA connection & measurement
Data capture (S-parameters)
Screen screenshot capture (:MMEMory:STORe:IMAGe)
Pass/Fail analysis (customizable)
Professional PDF report generation with reportlab
Unique PDF filename per test: Report_{Serial}_{Part}_{Order}_{timestamp}.pdf
...
How to UseInstall dependencies:bash

pip install pyvisa pyvisa-py numpy reportlab matplotlib

Run the script:bash

python main.py

First run: It will list available VISA resources. Note the address for your MS46122B.
Customize:Edit DEFAULT_LIMITS for your specific cable/part requirements.
Add more S-parameters (S11, S12, S22) in trigger_and_read_data().
Improve Pass/Fail logic as needed.
